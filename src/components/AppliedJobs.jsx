...
import RemoteJob from './FilteredJobs/RemoteJob';
import OnsiteJob from './FilteredJobs/OnsiteJob';
        ...
        <div>
            ...
            <div className='container px-5 py-24 mx-auto items-center'>
                ...
                <div className='flex justify-center items-center'>
                    <div className='mb-5'>
                        <button onClick={() => setJobType("Remote")} type="button" className={`border border-indigo-500 hover:bg-indigo-500 rounded-lg py-2 px-4 font-semibold ml-2 focus:outline-none focus:shadow-outline mb-4 sm:mb-0 ${jobType == "Remote" ? "bg-indigo-500 text-white": ""}`}>Remote</button>
                        <button onClick={() => setJobType("Onsite")} type="button" className={`border border-indigo-500 hover:text-white text-indigo-500 hover:bg-indigo-700 rounded-lg py-2 px-4 font-semibold ml-2 focus:outline-none focus:shadow-outline mb-4 sm:mb-0 ${jobType == "Onsite" ? "bg-indigo-500 text-white": ""}`}>Onsite</button>
                    </div>
                </div>
                ...
                <section className="text-gray-600 body-font">
                    <div className="items-center">
                        ...
                        <div className="border rounded-md w-4/5 p-4 mx-auto ">
                            {jobType == "Remote" &&
                                <div>
                                {RemoteJobsList.map(job => <RemoteJob key={job.id} job={job}></RemoteJob>)}
                            </div>}

                            {jobType == "Onsite" &&
                                <div>
                                {onsiteJobsList.map(job => <OnsiteJob key={job.id} job={job}></OnsiteJob>)}
                            </div>}
                        </div> 
                        }
